...
ENNB can run multiple group analysis or two group.  For this comparison we subset the groups into the 3 combinations possible for two group analyses.
```{r}
...
The following function builds a confusion matrix and calculates the false discovery rate.  The user supplies the name of a dataframe for the two group comparison.  

Example: *calc_confusion_mat( **gf_a_b** )*
```{r}
...
Run the ENNB workflow to find the significantly different features between groups.  Only TMM option **1** works for this data.  I added an adjustment for threshold.  Notice the naming conventions are the same as the two group dataframes.  This is essential for the confusion matrix function to work.
```{r}
TwoStage_Package(gf_a_b[,-c(1,3)], factors_a_b, "gf_a_b_tmm1.csv", TMM.option = 1, threshold = .001)
TwoStage_Package(gf_a_c[,-c(1,3)], factors_a_b, "gf_a_c_tmm1.csv", TMM.option = 1, threshold = .001)
TwoStage_Package(gf_b_c[,-c(1,3)], factors_b_c, "gf_b_c_tmm1.csv", TMM.option = 1, threshold = .001)
```

Using the output from the ENNB workflow we can now caculate the false discovery rate for each of the two group comparisons.
```{r, message=FALSE}
...
The output from group a & c is clearly our best as we have a low number of false positives.
