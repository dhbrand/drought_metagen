ennb <- function(X,Y,fileoutput, TMM.option, threshold){
  ...
  ennb <- function(X, Y, fileoutput){
    ...
    elast = as.matrix(elast[-1, ]) #get rid of intercept
    ...
    sig.mat = as.matrix(pvalue.mat[pvalue.mat[,"p.adj"] < threshold,]) # Significantly differentially abundant features  
    ...
    sig.df = data.frame(as.character(rownames(X.sig)), mean.pop1, sd.pop1, mean.pop2, sd.pop2, sig.mat[,"p.val"], sig.mat[,"p.adj"], stringsAsFactors=F)
  ...
  if (ngroups == 2) ennb(X, Y, fileoutput)
